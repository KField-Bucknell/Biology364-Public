...
- She removed: S69_AMY_S101, S77_PVN_S57, S80_AMY_S106, S95_PVN_S63
- If you identify any additional outliers in your analysis, first contact the other group(s) that are studying the same samples. If you both agree that a sample is an outlier, then notify Prof. Field and remove that sample. If you don't both agree, then the sample should remain in both datasets.

5. Summarize the numbers of remaining samples to determine completeness

6. Analyze differential expression by treatment, isolation, or tissue within this group of samples
...
- padj value cut off:
- foldChange cut off:
- Tables to download and filename conventions: See Project3.md
- Figures to download, figure size, figure format, and filename conventions: See Project3.md
...
veh_Eiso_BNST_metadata <- read_csv("voles/veh.Eiso.BNST_metadata.csv")
...
iso_Eoxt_BNST_metadata <- read_csv("voles/iso.Eoxt.BNST_metadata.csv")
head(iso_Eoxt_BNST_metadata)
iso_Eoxt_BNST_metadata %>%
  select(Isolation, Treatment) %>%
  table
# Effect of isolation and oxytocin treatment together
Eiso_oxt_BNST_metadata <- read_csv("voles/Eiso+oxt.BNST_metadata.csv")
...
columns. 
Don't forget that you also need the `gene_ensm` column to be the first column of the genetable.
Then save that csv for your DEBrowser analysis.

```{r}
ALL_genetable <- read_csv("voles/ALL_genetable.csv")
veh_Eiso_BNST_genetable <- ALL_genetable %>%
  select(gene = gene_ensm, veh_Eiso_BNST_metadata$sample)
write_csv(veh_Eiso_BNST_genetable, file = "voles/veh_Eiso_BNST_genetable.csv")

iso_Eoxt_BNST_genetable <- ALL_genetable %>%
  select(gene = gene_ensm, iso_Eoxt_BNST_metadata$sample)
write_csv(iso_Eoxt_BNST_genetable, file="voles/iso_Eoxt_BNST_genetable.csv")
```
