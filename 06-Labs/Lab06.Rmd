...
vaccine_selected <- vaccine %>%
  select(FIPS, State, Recip_County, Metro_status, SVI_CTGY, matches("18Plus")) 
...
Before we can merge the two data frames, we need to identify the columns that match.

FIPS and county_fips are the unique identifiers for each county in the two data frames.

And then we will use left_join() from dplyr to add the state names to that data frame.

```{r}
combo <- vaccine_filter %>%
  left_join(voting, by = join_by(FIPS == county_fips))
glimpse(combo)
```

Compare the results of inner_join to left_join and full_join to decide which one is correct for this situation.

```{r}

```

It is handy to visualize missing data for this purpose

Explore the missing data so that we can determine if we should clean it up.
...
The following approach drops any row with even a single NA. Use it cautiously.

```{r}
covid_complete <- combo %>%
  drop_na()
sum(is.na(covid_complete))
```
...
ggplot(covid_complete, aes(x=Series_Complete_18PlusPop_Pct, 
                              y=per_dem,
                              color=as.factor(cutree(fullClustering, h=50)))) +
  geom_point() +
  scale_colour_discrete(name  = "Cluster") +
  theme_cowplot()
```
...
ann_colors <- list(State = as.factor(covid_complete$State))
covid.matrix <- as.matrix(scaled_covid)
rownames(covid.matrix) <- c(1:nrow(covid_complete)) # to annotate, matrix and ann_row must have same rownames
ann_row <- data.frame(State = as.factor(covid_complete$State))
...
```{r}
ggbiplot(covid.pca, choices = c(1,2), obs.scale = 1, var.scale = 1, 
